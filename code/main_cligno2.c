...
//
// Modification         : --> corriger la valeur du timer1  
//
// Remarques			: 	
...
int lecture_S_Avant; 								// indice pour la touche (AVANT delay)					
int lecture_S_Apres; 								// indice pour la touche (APRES delay)
int compteur; 
...
void main()
{
	...
	while(1)
	{
		//--- test sur le switch S3 appuyer ---//
		if(PORTDbits.RD6 == 1)
			lecture_S_Avant = 1;		
		
		//--- test sur le compteur = 500ms ---// 
		if(compteur == 5)
			//--- test sur le switch S3 ---// 
			if(PORTDbits.RD6 == 1)
			{
				lecture_S_Apres = 1; 
				if(lecture_S_Avant == lecture_S_Apres)
					indice_led++;	
			}
	}	
}
...
void init_timer1(void)
{
	...
	PR1 = 3125; 				// période du timer --> 100ms/32us = 3125 
